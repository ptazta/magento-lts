			...
			if(typeof(rev_sliders_info) !== 'undefined'){
				for(var key in rev_sliders_info) if (rev_sliders_info.hasOwnProperty(key)) {
					if(key == sel.data('sliderid')){
						...
						for(var okey in revslider_vc_variables['order']) if (revslider_vc_variables['order'].hasOwnProperty(okey)) {
							for(var sl in my_entry) if (my_entry.hasOwnProperty(sl)) {
						...
						for(var okey in revslider_vc_variables['order']) if (revslider_vc_variables['order'].hasOwnProperty(okey)) {
							for(var sl in my_entry) if (my_entry.hasOwnProperty(sl)) { //add all found slides, empty the old ones first
								if(my_entry[sl]['slider_type'] == 'hero'){
						...
						for(var slm in my_entry) if (my_entry.hasOwnProperty(slm)) {
						...
						for(var ekey in my_entries) if (my_entry.hasOwnProperty(ekey)) {
